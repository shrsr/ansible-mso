...
def main():
    ...
    if state == "present":
        if match:
            ...
            if request_data_size is not None and match.details.get("reqDataSize") != request_data_size:
                ops.append(
                    dict(
                        op="replace",
                        path="{0}/{1}/reqDataSize".format(path, match.index),
                        value=request_data_size,
                    )
                )
                match.details["reqDataSize"] = request_data_size
            ...
            if request_data_size:
                payload["reqDataSize"] = request_data_size
